# Validation — ThrottleAT 0.5.2-test CE059 / LCP experimental
...
0.5.2 regression coverage is described in CHANGES-052.md. User long-term feedback reports that the previous CVT works; new persistence and stepped-AT policies still need driving confirmation.

Full-pedal latch regression additionally covers repeated contact loss, same-vehicle controller reset, time discontinuity and fresh confirmation when changing from ordinary downshift to kickdown. The economy timing bound accounts for the configured confirmation interval plus frame quantization.
