# Validation — ThrottleAT 0.3.0 CE059 / LCP experimental

Toolchain: Visual Studio 2022 Build Tools, MSVC Win32 Release, static C/C++ runtime. The source import is built and tested again before publishing.
...
- ce059_bridge: 34 checks using allocated synthetic vehicle/player/pool/handling memory. Includes actual adapter writes, player ownership, lifecycle gaps, notifications, LCP frame translation, early first upshift with launch clutch slip, unchanged native revs, and missing-frame pass-through.
...
Earlier user driving logs establish the LCP route conflict; they do not validate the new integration. Actual 0.3.0 driving, LVS display RPM, 30/60+ FPS behavior, load/pause/vehicle changes, and crash-free gameplay remain unverified. The mock and CPU-state tests do not prove in-game compatibility. See LCP-COMPATIBILITY.md and CE059-EVIDENCE.md for exact evidence and limits.
