# ThrottleAT 0.5.0-test — GTA IV 1.2.0.59 Windows

바이크를 스포츠, 크루저, 일반, 스쿠터로 나누고 Faggio의 엔진 계산에 연속 기어비를 연결한 실험 브랜치입니다. 자동차 설정은 0.4.1과 같고 Sultan 전용 보정은 없습니다. main의 0.4.1과 별도로 시험합니다.

MSVC Win32 빌드와 자동 검사 9개가 통과했습니다. 실제 게임 주행과 LVS 대시보드 RPM은 아직 검증하지 않았습니다. [바이크 설정 및 CVT 구현 근거](docs/CVT-050-EVIDENCE.md), [LibertyCityPlates 연결 근거](docs/LCP-COMPATIBILITY.md)를 참고하세요.

Windows / GTA IV 1.2.0.59 대상입니다. ScriptHook, ScriptHookDotNet, .NET 런타임은 필요하지 않습니다. 실행 파일 전체 해시는 검사하지 않으며 버전과 관련 명령어를 검증합니다.
...
스로틀 0~30%는 완만한 저회전 구간, 30~65%는 중간 수요 구간, 65~100%는 고회전 구간입니다. 각 구간 사이를 연속적으로 보간합니다. Sultan을 포함한 일반 승용차의 Low/Mid/High는 0.22/0.54/0.94, Down은 0.07입니다. 지속 스로틀 20% 업시프트 기준은 약 0.247입니다. Sultan 전용 설정은 없습니다.
...
클래스 프리셋 -> `[Class:...]` 설정 -> `[Model:...]` 설정 순으로 적용합니다. 차량명은 게임의 vehicles.ide에서 읽고 모델 해시와 연결합니다. 트럭/버스와 스포츠카의 대표 모델 목록을 분류하며, 나머지는 승용차 프리셋입니다. 이는 실제 차량 제원을 추정하는 분류기가 아닙니다. 바이크는 검증한 게임 내부 타입을 확인한 뒤 아래 모델 분류를 적용합니다. 알 수 없는 바이크는 StandardBike입니다. 이전 `[Class:Motorcycle]` 설정은 모든 바이크의 공통 바탕으로 읽고, 구체적인 클래스 설정과 모델 설정으로 덮어씁니다. 모델 이름은 대소문자를 구분하지 않고 나머지 키는 정확히 입력해야 합니다.
...
| 클래스 | 대표 모델 | Low / Mid / High | 변속 후 대기 |
|---|---|---|---|
| SportBike | AKUMA, BATI, HAKUCHOU, NRG900 | 0.40 / 0.78 / 0.98 | 0.30초 |
| CruiserBike | FREEWAY, HEXER, ZOMBIE | 0.26 / 0.56 / 0.90 | 0.60초 |
| StandardBike | PCJ, VADER, SANCHEZ, 미분류 바이크 | 0.32 / 0.68 / 0.97 | 0.40초 |
| Scooter | FAGGIO | 0.27 / 0.55 / 0.88 (유단 AT용) | 0.50초 |

수치는 정규화된 기준입니다. 특정 실제 RPM을 보장하지 않습니다. 유단 바이크는 공통 스로틀 필터·킥다운·히스테리시스 로직에 각 프리셋을 적용합니다. 사용자 추가 바이크는 `[Model:모델명]`의 `BikeClass`로 분류할 수 있습니다.

Faggio는 `[Class:Scooter] Cvt=1`이 기본입니다. F8로 켜면 원래 정수 기어를 유지하고, 스로틀에 따른 목표 회전 신호에 맞춰 가상 기어비를 연속 변경합니다. RPM·속도 제한·구동력 계산에 같은 기어비를 전달하며, 원본 기어비 범위와 변화 속도 제한을 사용합니다. 게임의 클러치와 엔진 반응은 유지합니다. 완전한 벨트/원심 클러치 시뮬레이션은 아닙니다.

실제 RPM과 구동력 계산이 기어비를 소비하면 `ThrottleAT: CVT TEST` 알림과 `CVT ENGINE COUPLED` 로그가 나옵니다. 이는 경로 진입 확인이며 주행 품질 검증은 아닙니다. CVT 중 기어 표시가 고정되는 것은 의도한 동작입니다.

CVT를 끄려면 게임 종료 후 `Cvt=0`으로 바꾸세요. **ThrottleAT의 Scooter 유단 AT 설정**을 사용합니다. CVT 명령어 검증/생성만 실패해도 같은 프리셋을 사용합니다. 차량 정보나 기본 제어 경로 자체가 안전 조건을 만족하지 못하면 원본 처리로 넘깁니다. 점프·한 바퀴 주행처럼 접지가 끊기거나 F8을 끌 때 원래 기어비로 돌아가므로 회전수/구동력 변화가 클 수 있습니다. 먼저 평지 저속에서 시험하세요.

CSV의 `control_cvt` 행은 CVT 요청, `cvt_ratio`는 가상 기어비, `cvt_target`은 정규화 목표입니다. 이 행의 `filtered_throttle`은 CVT가 사용하는 원래 스로틀이며 `up_threshold`와 `down_threshold`는 유단 AT 참고값입니다. `class_id` 4/5/6/7은 스포츠/크루저/일반/스쿠터입니다.
...
- `src/cvt.cpp`, `src/x86_cvt.cpp`: 연속 기어비 제어와 검증된 엔진 계산 연결.
