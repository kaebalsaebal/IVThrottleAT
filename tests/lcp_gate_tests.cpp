// Synthetic MSVC/x86 test of the actual production LibertyCityPlates compatibility gate, without GTA IV or a
...
extern "C" __declspec(naked) int __cdecl atLcpDispatch(
...
int main() {
    atLcpGateway = reinterpret_cast<void*>(&fallbackLanding);
    atLcpSkipTarget = reinterpret_cast<void*>(&skipLanding);
    for (decision = 0; decision != 2; ++decision) {
        for (unsigned df = 0; df != 2; ++df) {
            ...
            check(dispatchFramePointer == 0x07070707, "dispatcher EBP frame pointer");
    ...
    if (failures) {
        std::printf("%d LCP x86 gate checks failed.\n", failures);
        return 1;
    }
    std::puts("PASS: 4 LCP x86 gate paths preserve stack, GPRs, EFLAGS, XMM, x87 and MXCSR.");
    return 0;
}
