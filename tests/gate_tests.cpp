...
extern "C" __declspec(naked) int __cdecl atDispatch(
...
int main() {
    atGateway = reinterpret_cast<void*>(&fallbackLanding);
    atSkipTarget = reinterpret_cast<void*>(&skipLanding);
    for (decision = 0; decision != 2; ++decision) {
        for (unsigned df = 0; df != 2; ++df) {
            ...
            check(dispatchTransmission == 0x03030303 &&
                  dispatchHandling == 0x05050505, "dispatcher ECX/ESI arguments");
