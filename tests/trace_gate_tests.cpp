// Synthetic MSVC/x86 test of the actual production gate, without GTA IV or a
...
std::uint32_t dispatchStackPointer{}, dispatchStack[10]{}, actualStack[10]{};
std::uint32_t decision{}, landing{};
Capture actual{};
...
extern "C" __declspec(naked) void __cdecl atTraceEntry(
    std::uintptr_t, std::uintptr_t, const std::uint32_t*) noexcept {
    __asm {
        pushfd
        pop [dispatchFlags]
        fxsave [dispatchFx]
        ...
        mov eax, [esp+12]
        mov [dispatchStackPointer], eax
        mov edx, [eax]
        mov [dispatchStack], edx
        mov edx, [eax+4]
        mov [dispatchStack+4], edx
        mov edx, [eax+8]
        mov [dispatchStack+8], edx
        mov edx, [eax+12]
        mov [dispatchStack+12], edx
        mov edx, [eax+16]
        mov [dispatchStack+16], edx
        mov edx, [eax+20]
        mov [dispatchStack+20], edx
        mov edx, [eax+24]
        mov [dispatchStack+24], edx
        mov edx, [eax+28]
        mov [dispatchStack+28], edx
        mov edx, [eax+32]
        mov [dispatchStack+32], edx
        mov edx, [eax+36]
        mov [dispatchStack+36], edx
        fninit
...
int main() {
    atShiftEntryGateway = reinterpret_cast<void*>(&fallbackLanding);
    atUpdateEntryGateway = reinterpret_cast<void*>(&fallbackLanding);
    for (decision = 0; decision != 2; ++decision) {
        traceTestGate = decision ? reinterpret_cast<void*>(&atUpdateEntryGate) : reinterpret_cast<void*>(&atShiftEntryGate);
        for (unsigned df = 0; df != 2; ++df) {
            seedFlags = 0x8d5u | (df << 10);
            landing = 0;
            runGate();
            check(landing == 1, "correct fallback/skip destination");
            ...
            check(dispatchTransmission == decision &&
                  dispatchHandling == 0x03030303, "dispatcher ECX/ESI arguments");
            check(dispatchStackPointer == expectedEsp, "dispatcher originalStack pointer");
            for (unsigned i = 0; i != 10; ++i) {
                check(dispatchStack[i] == 0xa0000000u + i, "dispatcher stack contents");
    ...
    if (failures) {
        std::printf("%d x86 gate checks failed.\n", failures);
        return 1;
    }
    std::puts("PASS: 4 x86 gate paths preserve stack, GPRs, EFLAGS, XMM, x87 and MXCSR.");
    return 0;
}
